/* eslint-disable @typescript-eslint/no-require-imports */
const { PrismaClient } = require("@prisma/client");

const db = new PrismaClient();

const MATH_TOPICS = [
  "1. Ünite: Doğal Sayılarla İşlemler",
  "2. Ünite: Çarpanlar ve Katlar, Kümeler",
  "3. Ünite: Tam Sayılar ve Kesirlerle İşlemler",
  "4. Ünite: Ondalık Gösterim ve Oran",
  "5. Ünite: Cebirsel İfadeler ve Veri Analizi",
  "6. Ünite: Açılar ve Alan Ölçme",
  "7. Ünite: Çember, Geometrik Cisimler ve Hacim"
];

const SCIENCE_TOPICS = [
  "1. Ünite: Güneş Sistemi ve Tutulmalar",
  "2. Ünite: Vücudumuzdaki Sistemler",
  "3. Ünite: Kuvvet ve Hareket",
  "4. Ünite: Madde ve Isı",
  "5. Ünite: Ses ve Özellikleri",
  "6. Ünite: Vücudumuzdaki Sistemler ve Sağlığı",
  "7. Ünite: Elektriğin İletimi"
];

const SOCIAL_TOPICS = [
  "1. Ünite: Birey ve Toplum",
  "2. Ünite: Kültür ve Miras",
  "3. Ünite: İnsanlar, Yerler ve Çevreler",
  "4. Ünite: Bilim, Teknoloji ve Toplum",
  "5. Ünite: Üretim, Dağıtım ve Tüketim",
  "6. Ünite: Etkin Vatandaşlık",
  "7. Ünite: Küresel Bağlantılar"
];

(async () => {
  try {
    console.log("📚 Grade 6 Konuları Oluşturuluyor...\n");

    let created = 0;

    // Create Matematik topics
    for (const topicName of MATH_TOPICS) {
      const existing = await db.curriculumTopic.findFirst({
        where: {
          gradeLevel: 6,
          subject: "MATEMATIK",
          name: topicName
        }
      });

      if (!existing) {
        await db.curriculumTopic.create({
          data: {
            gradeLevel: 6,
            subject: "MATEMATIK",
            name: topicName,
            description: topicName
          }
        });
        console.log(`   ✓ Oluşturuldu: [MATEMATIK] ${topicName}`);
        created++;
      }
    }

    // Create Science topics
    for (const topicName of SCIENCE_TOPICS) {
      const existing = await db.curriculumTopic.findFirst({
        where: {
          gradeLevel: 6,
          subject: "FEN_BILIMLERI",
          name: topicName
        }
      });

      if (!existing) {
        await db.curriculumTopic.create({
          data: {
            gradeLevel: 6,
            subject: "FEN_BILIMLERI",
            name: topicName,
            description: topicName
          }
        });
        console.log(`   ✓ Oluşturuldu: [FEN_BILIMLERI] ${topicName}`);
        created++;
      }
    }

    // Create Social Studies topics
    for (const topicName of SOCIAL_TOPICS) {
      const existing = await db.curriculumTopic.findFirst({
        where: {
          gradeLevel: 6,
          subject: "SOSYAL_BILGILER",
          name: topicName
        }
      });

      if (!existing) {
        await db.curriculumTopic.create({
          data: {
            gradeLevel: 6,
            subject: "SOSYAL_BILGILER",
            name: topicName,
            description: topicName
          }
        });
        console.log(`   ✓ Oluşturuldu: [SOSYAL_BILGILER] ${topicName}`);
        created++;
      }
    }

    console.log(`\n✅ ${created} yeni konu oluşturuldu!\n`);
    await db.$disconnect();
  } catch (error) {
    console.error("❌ Hata:", error);
    process.exit(1);
  }
})();
