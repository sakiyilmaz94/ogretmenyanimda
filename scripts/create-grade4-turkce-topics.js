...
const TURKCE_TOPICS = [
  ...
  "6. Ünite: SÖZCÜK VE SÖZCÜK GRUPLARI",
  "7. Ünite: NOKTALAMA İŞARETLERİ VE YAZIM KURALLARI",
];

(async () => {
  try {
    console.log("📚 Grade 4 Türkçe Konuları Oluşturuluyor...\n");

    // Eski (sorusuz, sınav-dışı) Grade 4 Türkçe konularını temizle
    ...
    for (const topicName of TURKCE_TOPICS) {
      const existing = await db.curriculumTopic.findFirst({
        where: { gradeLevel: 4, subject: "TURKCE", name: topicName },
      });
      if (!existing) {
